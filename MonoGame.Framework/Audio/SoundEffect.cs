...
namespace Microsoft.Xna.Framework.Audio
{
    ...
    /// </summary>
    /// <remarks>
    /// <para>A SoundEffect represents the buffer used to hold audio data and metadata. SoundEffectInstances are used to
    /// play from SoundEffects. Multiple SoundEffectInstance objects can be created and played from the same SoundEffect
    /// object.</para> <para>The only limit on the number of loaded SoundEffects is restricted by available memory. When
    /// a SoundEffect is disposed, all SoundEffectInstances created from it will become invalid.</para>
    /// <para>SoundEffect.Play() can be used for 'fire and forget' sounds. If advanced playback controls like volume or
    /// pitch is required, use SoundEffect.CreateInstance().</para>
    /// </remarks>
    public sealed partial class SoundEffect : IDisposable
    {
        ...
        internal SoundEffect(byte[] header,
                             byte[] buffer,
                             int bufferSize,
                             int durationMs,
                             int loopStart,
        ...
        internal SoundEffect(MiniFormatTag codec,
                             byte[] buffer,
                             int channels,
                             int sampleRate,
                             int blockAlignment,
        ...
        /// <summary>
        /// Initializes the sound system for SoundEffect support. This method is automatically called when a SoundEffect
        /// is loaded, a DynamicSoundEffectInstance is created, or Microphone.All is queried. You can however call this
        /// method manually (preferably in, or before the Game constructor) to catch any Exception that may occur during
        /// the sound system initialization (and act accordingly).
        /// </summary>
...
/// </summary>
        ...
        public SoundEffect(byte[] buffer, int sampleRate, AudioChannels channels) : this(buffer,
                                                                                         0,
                                                                                         buffer != null
            ? buffer.Length
            : 0,
                           ...
                           int loopLength)
        {
            ...
            if((buffer.Length % blockAlign) != 0)
                throw new ArgumentException("Ensure that the buffer meets the block alignment requirements for the number of channels.",
                                            nameof(buffer));

            if(count <= 0)
                throw new ArgumentException("Ensure that the count is greater than zero.", nameof(count));
            if((count % blockAlign) != 0)
                throw new ArgumentException("Ensure that the count meets the block alignment requirements for the number of channels.",
                                            nameof(count));

            if(offset < 0)
                throw new ArgumentException("The offset cannot be negative.", nameof(offset));
            if(((ulong)count + (ulong)offset) > (ulong)buffer.Length)
                throw new ArgumentException("Ensure that the offset+count region lines within the buffer.",
                                            nameof(offset));
            ...
            if(loopStart > totalSamples)
                throw new ArgumentException("The loopStart cannot be greater than the total number of samples.",
                                            nameof(loopStart));
            ...
            if(((ulong)loopStart + (ulong)loopLength) > (ulong)totalSamples)
                throw new ArgumentException("Ensure that the loopStart+loopLength region lies within the sample range.",
                                            nameof(loopLength));
...
/// cref="Microsoft.Xna.Framework.Audio.SoundEffect"/> is reclaimed by garbage collection.
/// </summary>
        ~SoundEffect() { Dispose(false); }
...
/// </summary>
        ...
        /// <remarks>
/// Creating a SoundEffectInstance before calling SoundEffectInstance.Play() allows you to access advanced
/// playback features, such as volume, pitch, and 3D positioning.
/// </remarks>
        ...
        /// <summary>
        /// Creates a new SoundEffect object based on the specified data stream. This internally calls <see
        /// cref="FromStream"/>.
        /// </summary>
        /// <param name="path">The path to the audio file.</param>
        /// <returns>The <see cref="SoundEffect"/> loaded from the given file.</returns>
        /// <remarks>
        /// The stream must point to the head of a valid wave file in the RIFF bitstream format.  The formats supported
        /// are: <list type="bullet">/// <item>/// <description>8-bit unsigned PCM</description>        ///
        /// <description>16-bit signed PCM</description>        /// <description>24-bit signed PCM</description>     ///
        /// <description>32-bit IEEE float PCM</description>        /// <description>MS-ADPCM 4-bit
        /// compressed</description>        /// <description>IMA/ADPCM (IMA4) 4-bit compressed</description>       
        /// ///</item>        ///</list>
        /// </remarks>
...
/// <para>To loop a sound or apply 3D effects, call SoundEffect.CreateInstance() and SoundEffectInstance.Play()
/// instead.</para> <para>SoundEffectInstances used by SoundEffect.Play() are pooled internally.</para>
/// </remarks>
        public bool Play()
        {
        ...
        /// <remarks>
        /// <para>Play returns false if more SoundEffectInstances are currently playing then the platform allows.</para>
        /// <para>To apply looping or simulate 3D audio, call SoundEffect.CreateInstance() and
        /// SoundEffectInstance.Play() instead.</para> <para>SoundEffectInstances used by SoundEffect.Play() are pooled
        /// internally.</para>
        /// </remarks>
...
/// </summary>
        ...
        /// </summary>
        /// <remarks>
        /// <para>Each SoundEffectInstance has its own Volume property that is independent to SoundEffect.MasterVolume.
        /// During playback SoundEffectInstance.Volume is multiplied by SoundEffect.MasterVolume.</para> <para>This
        /// property is used to adjust the volume on all current and newly created SoundEffectInstances. The volume of
        /// an individual SoundEffectInstance can be adjusted on its own.</para>
        /// </remarks>
        ...
        /// </summary>
        /// <remarks>
        /// <para>DistanceScale defaults to 1.0 and must be greater than 0.0.</para> <para>Higher values reduce the rate
        /// of falloff between the sound and listener.</para>
        /// </remarks>
        ...
        /// </summary>
        /// <remarks>
        /// <para>DopplerScale defaults to 1.0 and must be greater or equal to 0.0</para> <para>Affects the relative
        /// velocity of emitters and listeners.</para> <para>Higher values more dramatically shift the pitch for the
        /// given relative velocity of the emitter and listener.</para>
        /// </remarks>
        ...
        /// </summary>
        /// <remarks>
        /// <para>Defaults to 343.5. Value is measured in meters per second.</para> <para>Has no effect on distance
        /// attenuation.</para>
        /// </remarks>
...
/// </summary>
        ...
        /// <remarks>
        /// If the disposing parameter is true, the Dispose method was called explicitly. This means that managed objects
        /// referenced by this instance should be disposed or released as required.  If the disposing parameter is false,
        /// Dispose was called by the finalizer and no managed objects should be touched because we do not know if they are
        /// still valid or not at that time.  Unmanaged resources should always be released.
        /// </remarks>
