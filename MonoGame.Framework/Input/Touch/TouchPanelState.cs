...
namespace Microsoft.Xna.Framework.Input.Touch
{
    public class TouchPanelState
    {
        ...
        /// <summary>
        /// The next touch location identifier. The value 1 is reserved for the mouse touch point.
        /// </summary>
        ...
        internal TouchPanelState(GameWindow window) { Window = window; }
        ...
        /// <summary>
        /// Apply the given new touch to the state. If it is a Pressed it will be added as a new touch, otherwise we
        /// update the existing touch it matches
        /// </summary>
        ...
        private void UpdateTouchScale()
        {
            ...
            _touchScale = new Vector2(_displaySize.X / windowSize.X, _displaySize.Y / windowSize.Y);
        }

        /// <summary>
        /// This will release all touch locations.  It should only be  called on platforms where touch state is reset
        /// all at once.
        /// </summary>
...
/// </summary>
        ...
        private bool GestureIsEnabled(GestureType gestureType) { return (EnabledGestures & gestureType) != 0; }
            ...
            foreach(var touch in _gestureState)
            {
                switch(touch.State)
                {
                    ...
                    case TouchLocationState.Released:
                    {
                        ...
                        // the pinch gesture operation.
                            if(_pinchGestureStarted && (touch.Id == _pinchTouch[0].Id || touch.Id == _pinchTouch[1].Id))
                            {
                                if(GestureIsEnabled(GestureType.PinchComplete))
                                    GestureList.Enqueue(new GestureSample(GestureType.PinchComplete,
                                                                          touch.Timestamp,
                                                                          Vector2.Zero,
                                                                          Vector2.Zero,
                                                                          Vector2.Zero,
                        ...
                        {
                            GestureList.Enqueue(new GestureSample(GestureType.Flick,
                                                                  touch.Timestamp,
                                                                  Vector2.Zero,
                                                                  Vector2.Zero,
                                                                  touch.Velocity,
        ...
        private void ProcessHold(TouchLocation touch)
        {
            ...
            GestureList.Enqueue(new GestureSample(GestureType.Hold,
                                                  touch.Timestamp,
                                                  touch.Position,
                                                  Vector2.Zero,
                                                  Vector2.Zero,
        ...
        private bool ProcessDoubleTap(TouchLocation touch)
        {
            ...
            GestureList.Enqueue(new GestureSample(GestureType.DoubleTap,
                                                  touch.Timestamp,
                                                  touch.Position,
                                                  Vector2.Zero,
                                                  Vector2.Zero,
        ...
        private void ProcessTap(TouchLocation touch)
        {
            ...
            {
                var tap = new GestureSample(GestureType.Tap,
                                            touch.Timestamp,
                                            touch.Position,
                                            Vector2.Zero,
                                            Vector2.Zero,
 ...
 //then it could be a free drag if not classified.
            ...
            GestureList.Enqueue(new GestureSample(_dragGestureStarted,
                                                  touch.Timestamp,
                                                  touch.Position,
                                                  Vector2.Zero,
                                                  delta,
        ...
        private void ProcessPinch(TouchLocation[] touches)
        {
            ...
            GestureList.Enqueue(new GestureSample(GestureType.Pinch,
                                                  timestamp,
                                                  touches[0].Position,
                                                  touches[1].Position,
                                                  delta0,
