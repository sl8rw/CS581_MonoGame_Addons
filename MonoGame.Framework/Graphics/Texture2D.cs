...
namespace Microsoft.Xna.Framework.Graphics
{
    public partial class Texture2D : Texture
    {
        ...
        /// </summary>
        public Rectangle Bounds { get { return new Rectangle(0, 0, this.width, this.height); } }
        ...
        public Texture2D(GraphicsDevice graphicsDevice, int width, int height) : this(graphicsDevice,
                                                                                      width,
                                                                                      height,
                                                                                      false,
                                                                                      SurfaceFormat.Color,
        ...
        public Texture2D(GraphicsDevice graphicsDevice, int width, int height, bool mipmap, SurfaceFormat format) : this(graphicsDevice,
                                                                                                                         width,
                                                                                                                         height,
                                                                                                                         mipmap,
                                                                                                                         format,
        ...
        /// <summary>
        /// Creates a new texture array of a given size with a surface format and optional mipmaps. Throws
        /// ArgumentException if the current GraphicsDevice can't work with texture arrays
        /// </summary>
        /// <param name="graphicsDevice"></param>
        /// <param name="width"></param>
        /// <param name="height"></param>
        ...
        public Texture2D(GraphicsDevice graphicsDevice,
                         ...
                         int arraySize) : this(graphicsDevice,
                                               width,
                                               height,
                                               mipmap,
                                               format,
        ...
        internal Texture2D(GraphicsDevice graphicsDevice,
                           ...
                           bool mipmap,
                           SurfaceFormat format,
                           SurfaceType type) : this(graphicsDevice, width, height, mipmap, format, type, false, 1)
        { }
                            ...
                            int arraySize)
        {
            ...
            if(arraySize > 1 && !graphicsDevice.GraphicsCapabilities.SupportsTextureArrays)
                throw new ArgumentException("Texture arrays are not supported on this graphics device",
                                            nameof(arraySize));
        ...
        /// <summary>
        /// Changes the pixels of the texture Throws ArgumentNullException if data is null Throws ArgumentException if
        /// arraySlice is greater than 0, and the GraphicsDevice does not support texture arrays
        /// </summary>
        ...
        /// <summary>
        /// Retrieves the contents of the texture Throws ArgumentException if data is null, data.length is too short or
        ...
        /// <summary>
        /// Creates a <see cref="Texture2D"/> from a file, supported formats bmp, gif, jpg, png, tif and dds (only for
        /// simple textures). May work with other formats, but will not work with tga files. This internally calls <see
        /// cref="FromStream"/>.
        /// </summary>
        ...
        /// <remarks>
        /// Note that different image decoders may generate slight differences between platforms, but perceptually  the
        /// images should be identical.  This call does not premultiply the image alpha, but areas of zero alpha will
        ...
        /// <summary>
        /// Creates a <see cref="Texture2D"/> from a stream, supported formats bmp, gif, jpg, png, tif and dds (only for
        /// simple textures). May work with other formats, but will not work with tga files.
        /// </summary>
        /// <param name="graphicsDevice">The graphics device to use to create the texture.</param>
        ...
        /// <param name="width"></param>
        /// <param name="height"></param>
        public void SaveAsJpeg(Stream stream, int width, int height) { PlatformSaveAsJpeg(stream, width, height); }
        ...
        /// <param name="width"></param>
        /// <param name="height"></param>
        public void SaveAsPng(Stream stream, int width, int height) { PlatformSaveAsPng(stream, width, height); }
            ...
            where T : struct
        {
            ...
            if(Format.IsCompressedFormat())
            {
                ...
                checkedRect = new Rectangle(checkedRect.X & ~blockWidthMinusOne,
                                            checkedRect.Y & ~blockHeightMinusOne,
#if OPENGL
                                            // OpenGL only: The last two mip levels require the width and height to be
                                            // passed as 2x2 and 1x1, but there needs to be enough data passed to occupy
                    // a full block.
                    checkedRect.Width < blockWidth && textureBounds.Width < blockWidth
                    ? textureBounds.Width
                    : roundedWidth,
                                            checkedRect.Height < blockHeight && textureBounds.Height < blockHeight
            ...
            if(elementCount * tSize != dataByteSize)
                throw new ArgumentException($"elementCount is not the right size, " +
                    $"elementCount * sizeof(T) is {elementCount * tSize}, but data size is {dataByteSize}.",
                                            nameof(elementCount));
        }
