...
namespace Game1_MGtest
{
    static class FPSCounter
    {
        ...
        {
            if (_fpsIncrement.ContainsKey(index) == false)
            {
                _fpsValue[index] = 0;
            ...
            _fpsIncrement[index] += 1;
            if (_fpsTimer[index].Elapsed.TotalMilliseconds > 1000.0f)
            {
        ...
        public static void Render(GraphicsDevice graphics, Color color, float xpos = 16, float ypos = 16, int index = -1, string label = "FPS")
        {
            ...
            _batch.Begin(SpriteSortMode.Immediate, BlendState.AlphaBlend, SamplerState.PointClamp, DepthStencilState.Default, RasterizerState.CullNone, null, Matrix.Identity);
            ...
            private static char[] _characters = new char[]
        {
            ...
            public void Draw(SpriteBatch batch, string text, float xpos, float ypos, Color c)
            {
                ...
                for (int i = 0; i < text.Length; i++)
                {
                    ...
                    int charIndex = _characterMap[((int)text[i])];
